...
| read-only SQL gate stripped comments without tracking string literals → `SELECT 'a/*'; DELETE …; SELECT '*/b'` looked like one SELECT and smuggled a mutation past the gate; fixed by `_split_sql_statements` respecting quotes | `TestReadOnlyUser.test_readonly_sql_comment_in_literal_smuggling_blocked` |
| read-only SQL gate accepted any single `WITH …` statement as read-only, but a leading CTE can front a mutation (`WITH x AS (SELECT 1) DELETE FROM t` is valid in SQLite and MySQL); WITH is now conservatively rejected | `TestReadOnlyUser.test_readonly_sql_with_cte_blocked` |
| `format_yaml_value` double-quoted values without escaping `"`/`\` → a value needing quotes that also contained a quote/backslash (e.g. adhoc SQL `SELECT a, "b"`) emitted invalid YAML and the save was rejected | `TestYamlValueEscaping` |
...
| config diff dropped `mysql_users` `frontend`/`backend` from every comparison (to match runtime's split per-role rows), so a memory-only flag change was reported as in sync; split rows are now merged with the flags OR-ed (`mdb._merge_user_rows`) and the flags compared | `TestConfigDiffUserFlagChange` |
